...
const VisitorBarChart: React.FC<VisitorBarChartProps> = ({ barId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const zonesResponse = await fetch(`http://127.0.0.1:8000/api/v1/getAllZonesByBarId/${barId}`);
        const zonesData = await zonesResponse.json();
        const zoneIds = zonesData.zones.map((zone: any) => zone.zone_id);

        // ดึงข้อมูล VisitorHistories
        const visitorsResponse = await fetch("http://127.0.0.1:8000/api/v1/getAllVisitorHistories");
        const visitorsData = await visitorsResponse.json();

        // กรองข้อมูลตาม zone_id และย้อนหลัง 30 วัน
        const now = new Date();
        const filteredVisitors = visitorsData.visitor_histories.filter((history: any) => {
          const historyDate = new Date(history.date_time);
          const isWithin30Days = (now.getTime() - historyDate.getTime()) / (1000 * 60 * 60 * 24) <= 30;
  ...
  return (
    <BarChart width={1400} height={300} data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
      <CartesianGrid strokeDasharray="3 3" />
      <XAxis dataKey="hour" tick={{ fontSize: 12 }} />
      <YAxis tick={{ fontSize: 12 }} />
      <Tooltip />
      <Legend />
      <Bar dataKey="averageCount" fill="#8884d8" />
    </BarChart>
  );
};
