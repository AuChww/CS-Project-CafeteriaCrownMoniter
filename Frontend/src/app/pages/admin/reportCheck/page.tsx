...
const ReportCheck = () => {
    ...
    const updateStatus = async (id: number, status: Report["report_status"]) => {
        try {
            await axios.put(`http://127.0.0.1:8000/api/v1/updateReport/${id}`, { status });
            setReports((prev) =>
                prev.map((r) =>
                ...
                <tbody>
                    {reports.map((r) => (
                        ...
                        >
                            ...
                            <td className="text-center px-4 py-2">{getUserName(r.user_id)}</td>
                            <td className="text-center px-4 py-2">{getZoneName(r.zone_id)}</td>
                            <td className="text-center px-4 py-2">
                                <span
                                    className={`ml-2 px-2 py-1 rounded font-bold ${r.report_status === "pending"
                                        ? "text-yellow-400"
                            ...
                            <td className="flex justify-end gap-x-2 px-4 py-2 ">
                                ...
                                {(r.report_status === "rejected" || r.report_status === "resolved") && (
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
