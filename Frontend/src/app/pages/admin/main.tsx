...
export default function Admin() {
  ...
  const handleDelete = async (bar_id: number, bar_name: string) => {
    const confirmDelete = window.confirm(
      `Are you sure to delete Bar ${bar_id} : ${bar_name} ?`
    );
    ...
    <div className="container mx-auto p-4 bg-gray-100 w-full h-screen overflow-y-auto">
      ...
      <div className="grid grid-cols-1 mt-6 sm:grid-cols-4 lg:grid-cols-6 gap-6">
        {bars.map((bar) => (
          <div
            key={bar.bar_id}
            className="bg-white hover:scale-110 duration-300 rounded-lg shadow-lg  border border-gray-200 hover:bg-gray-100"
          >
            <AdminCrowdCard
              key={bar.bar_id}
              bar_id={bar.bar_id}
